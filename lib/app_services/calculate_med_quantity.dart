...
Future<void> calculateMedQuantities() async {
  ...
  for (final medication in medications) {
    ...
    if (!isTodaySameDay) {
      if (medication.takeQuantityPerDay != 0 &&
          _shouldTakeMedicationToday(medication, today)) {
        final newQuantity =
            medication.currentQuantity - medication.takeQuantityPerDay;
        final double updatedQuantity = newQuantity > 0 ? newQuantity : 0;

        final updatedTotalDoses = doseCalculator.calculateTotalDoses(
            updatedQuantity,
            medication.takeQuantityPerDay,
            medication.isAlternatingSchedule);
...
bool _shouldTakeMedicationToday(Medication medication, DateTime today) {
  if (medication.isAlternatingSchedule) {
    ...
    int daysSinceLastUpdate =
        today.difference(medication.lastUpdatedDate).inDays;
    ...
    switch (today.weekday) {
      case DateTime.monday:
        return medication.takeMonday;
      case DateTime.tuesday:
        return medication.takeTuesday;
      case DateTime.wednesday:
        return medication.takeWednesday;
      case DateTime.thursday:
        return medication.takeThursday;
      case DateTime.friday:
        return medication.takeFriday;
      case DateTime.saturday:
        return medication.takeSaturday;
      case DateTime.sunday:
        return medication.takeSunday;
      default:
        return false;
    }
  }
