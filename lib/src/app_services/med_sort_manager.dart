...
final medSortModeProvider = StateNotifierProvider<MedSortModeNotifier, AsyncValue<MedSortMode>>((ref) {
...
class MedSortModeNotifier extends StateNotifier<AsyncValue<MedSortMode>> {
  MedSortModeNotifier(this._medSortRepository) : super(const AsyncValue.loading()) {
    _loadSortMode();
  }
...
final showZeroDosesProvider = StateNotifierProvider<ShowZeroDosesNotifier, AsyncValue<bool>>((ref) {
  final repository = ref.watch(medSortRepositoryProvider);
  return ShowZeroDosesNotifier(repository);
});

class ShowZeroDosesNotifier extends StateNotifier<AsyncValue<bool>> {
  ShowZeroDosesNotifier(this._medSortRepository) : super(const AsyncValue.loading()) {
    _loadShowZeroDoses();
  }
...
List<Medication> sortMedications(List<Medication> meds, MedSortMode sortMode, bool showZeroDoses) {
  final filteredMeds = showZeroDoses ? meds : meds.where((med) => med.takeQuantityPerDay > 0).toList();
  
  switch (sortMode) {
    case MedSortMode.nameAZ:
      filteredMeds.sort((a, b) => a.name.toLowerCase().compareTo(b.name.toLowerCase()));
    case MedSortMode.nameZA:
      filteredMeds.sort((a, b) => b.name.toLowerCase().compareTo(a.name.toLowerCase()));
